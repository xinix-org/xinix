...
static inline size_t read_max_ents(struct DynldState* state) {
    auto val = atomic_load_explicit(&state->header.dynld_lock_and_offset, memory_order_acquire) & ~lock_bit;
    return val;
}
...
static inline ElfNative_Offset addend_rel(Elf64_Rel* r) {
    return 0;
}
...
static inline sysresult2_t apply_rel(void* offset, ElfNative_Reloc reloc, uint64_t addend, bool read_add, const Elf64_Sym* sym, bool resolve_defered, bool skip_non_deffered, const struct DynLibraryEntry* ent) {
    const Elf64_Sym* val;
    ...
    else if(sym->st_shndx && ((ST_BIND((*sym)) == STB_LOCAL) || (ST_VISIBILITY(*sym) != STV_DEFAULT))){
        val = sym;
        sym_ent = ent;
    }
    else {
    ...
    case DREL_JUMP_SLOT:
        {
            ...
            void* rrel = ((char*)sym_ent->dylib_base) + val->st_value + addend +  (read_add ? (size_t)(*addr) : 0);
    ...
    case DREL_RELATIVE:
        {
            void** addr = (void**)offset;
            void* rrel = ((char*)sym_ent->dylib_base) + addend +  (read_add ? (size_t)(*addr) : 0);
            *addr = rrel;
        }
        break;
    case DREL_IRELATIVE:
...
#define DO_REL(reltag, reladd_f, read_add, deferred)\
        do {\
            ...
            for(;_rel != _rel_end; _rel++)\
                {\
                    ...
                    /* sysresult2_t apply_rel(void* offset, ElfNative_Reloc reloc, uint64_t addend, bool read_add, const Elf64_Sym* sym, bool resolve_defered, bool skip_non_deffered, const struct DynLibraryEntry* ent) */\
                    auto res = apply_rel(_offset, _relty, _addend, _read_add, _sym, _needs_deferred, _needs_deferred, &ent);\
                    SYSRESULT_TRY_SYSRESULT2(SYSRESULT2_CODE(res));\
    ...
    while((ent_lock = atomic_fetch_or_explicit(&dynld_state.header.dynld_lock_and_offset, lock_bit, memory_order_acquire)) & lock_bit)
        spin_loop_hint();

    size_t ent_off = ent_lock & ~lock_bit;

    if(ent_off == (sizeof(dynld_state.entries) / sizeof(dynld_state.entries[0])))
        return SYSRESULT2_ERROR(-1);
