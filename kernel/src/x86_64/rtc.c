...
void init_rtc() {
    ...
    uint16_t leap_days = (leap_year_start / 4) - (leap_year_start / 100) + (leap_year_start / 400);

    int16_t epoch_leap_days = leap_days - LEAP_DAYS_1970;

    bool is_leap_year = !(leap_year_start & 3) && (!(leap_year_start %100) || !(leap_year_start % 400));
    ...
    printf("RTC: %0.4u-%0.2u-%0.2u %0.2u:%0.2u:%0.2uZ\r\n", year, values.months, values.days, values.hours, values.minutes, values.seconds);
...
duration_t read_global_rtc() {
    ...
    if (diff > cur_boottime) {
        printf("Panick: Boot time rolled backwards (RTC initialized at %w64d, current time %w64d)", boottime, cur_boottime);
   ...
   return duration_add(base_rtc, (duration_t){.time_seconds = seconds_delta, .time_nanos = nanos_delta});
}