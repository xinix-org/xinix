...
sysresult2_t create_context(struct thread *th, paddr_t ptable) {
    ...
    uint16_t default_code_seg =
        thread_is_kernel ? GDT_KCODE64 : (GDT_UCODE64 | 3);
    auto cr4 = read_cr4();
    ...
    ucontext_t *ctx =
        aligned_alloc(alignof(ucontext_t), sizeof(ucontext_t) + xsave_size);
    if (!ctx)
        return SYSRESULT2_ERROR(ERR_INSUFFICIENT_MEMORY);
    memset(ctx, 0, sizeof(ucontext_t) + xsave_size);

    ctx->xsave_size = xsave_size;
    ctx->xcr0_allowed = ((uint64_t)x86_feature_array[32]) |
                        ((uint64_t)x86_feature_array[33]) << 32;
