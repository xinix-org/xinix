...
static void print_sreg(const char* name, uint16_t sreg, uint16_t ldtr) {
    ...
    if (!test_flag(sreg, 2)) {
        ...
        uint64_t ldt_base = ((uint64_t)gdt_entries[ldtr].base_lo) | (((uint64_t)gdt_entries[ldtr].base_mid) << 16) | (((uint64_t)gdt_entries[ldtr].base_hi) << 24) | (((uint64_t)gdt_entries[ldtr+1].sys_base_ext)<<32);
        gdt_entry_t* ldt = (gdt_entry_t*)ldt_base;
        ent = &ldt[offset];
    }

    uint32_t limit = ((uint32_t)ent->limit_lo) | ((uint32_t)(ent->flags_and_limit_hi & 0xF)) << 16;

    uint64_t base = ((uint32_t)ent->base_lo) | (((uint32_t)ent->base_mid) << 16) | (((uint32_t)ent->base_hi) << 24);
    ...
    if(!sys) {
        ...
        printf("\t%s = %.4X [%s, base = %.16llX, limit = %.8X, P = %X, DPL=%X]\r\n", name, sreg, segty, present, dpl);
    } else {
        ...
        const char* segty = test_flag(ent->flags_and_limit_hi, 5) ? "64-bit" : (test_flag(ent->flags_and_limit_hi, 6) ? "32-bit" : "16-bit");

        if(x) {
            printf("\t%s = %.4X [%s code, base = %.8lX, limit = %.8X, CPL = %x, DPL = %x, P = %X, A = %X, R = %X, C = %X]\r\n", name, sreg, segty, base, limit, cpl, dpl, present, a, rw, dc);
        } else {
            printf("\t%s = %.4X [%s data, base = %.8lX, limit = %.8X, DPL = %x, P = %X, A = %X, W = %X, D = %X]\r\n", name, sreg, segty, base, limit, dpl, present, a, rw, dc);
        }
    }
}
...
void print_ucontext(const ucontext_t *context) {
    ...
    printf("\tFS.BASE = %p\tGS.BASE = %p\r\n", context->fsgsbase[0], context->fsgsbase[1]);

    printf("\tCR3 = %#.16llX\tCR4 = %#.16llX\r\n", context->cr3, context->cr4);
    printf("\t\tTSD = %X, DE = %X, MCE = %X, PGE = %X, PCE = %X, UIMP = %X, LA57 = %X\r\n", 
        test_flag(context->cr4, 2), 
        test_flag(context->cr4, 3), 
        test_flag(context->cr4, 6), 
        test_flag(context->cr4, 7), 
        test_flag(context->cr4, 8),
        test_flag(context->cr4, 11), 
        test_flag(context->cr4, 12));
    printf("\t\tVMXE = %X, SMXE = %X, FSGSBASE = %X, PCIDE = %X, SMEP = %X, SMAP = %X, PKE = %X, CET = %X, PKS = %X\r\n", 
            test_flag(context->cr4, 13), 
            test_flag(context->cr4, 14), 
            test_flag(context->cr4, 16), 
            test_flag(context->cr4, 17),
            test_flag(context->cr4, 20),
            test_flag(context->cr4, 21),
            test_flag(context->cr4, 22),
            test_flag(context->cr4, 23),
            test_flag(context->cr4, 24));

    printf("\t\tFXSR = %X, OSXMMEX = %X, XSAVE = %X\r\n", test_flag(context->cr4, 9), test_flag(context->cr4, 10), test_flag(context->cr4, 18));

    printf("\tDR0 = %.16p\tDR1 = %.16p\r\n", context->dregs[0], context->dregs[1]);
    printf("\tDR2 = %.16p\tDR3 = %.16p\r\n", context->dregs[2], context->dregs[3]);
    uintptr_t dr6 = (uintptr_t)context->dregs[4];
    printf("\tDR6 = %#.16lX [BP0 = %X, BP1 = %X, BP2 = %X, BP3 = %X, BLD = %X, BD = %X, BS = %X, BT = %X, RTM = %X]\r\n", dr6, test_flag(dr6, 0), test_flag(dr6, 1), test_flag(dr6, 2), test_flag(dr6, 3), test_flag(dr6, 11), test_flag(dr6, 13), test_flag(dr6, 14), test_flag(dr6, 15), !test_flag(dr6, 16));
    ...
    for(size_t n = 0; n < 4; n++)
        printf("\tBP%x = %s (%s, LOG=%X)", n, cond[n], en[n], test_flag(dr7, 32 + n));
    printf("\r\n");
    ...
    if(context->xsave_size >= FXSAVE_SIZE) {
        ...
        printf("\tFSW = %.4X [CC = %.2X, EX = [%s]]\tFCW = %.4X [RC = %s, PC = %s, EM = [%s]]\tFTW = %.2X\r\n", fsw, x87_cc, legacy_except_status, fcw, legacy_rc, legacy_pc, legacy_except_mask, ftw);
        printf("\tMXCSR = %.8X [FTZ = %X, DAZ = %X, RC = %s, EM = [%s], EX = [%s]]\r\n", mxcsr, sse_ftz, sse_daz, sse_rc, mxcsr_except_mask, mxcsr_except_status);
